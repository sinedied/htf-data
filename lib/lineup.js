  ...
  static import(lineupData, htfData, dbData, startDate) {
    ...
    _.each(lineup, (l, i) => {
      ...
      while ((match = setRegexp.exec(l))) {
        ...
        if (!isBreak && !artist) {
          // console.log(_.find(dbData, a => a.id === "153").name)
        ...
        const set = {
          type: type,
          start: start.subtract(lineupUtcOffset, 'h').toISOString(),
          end: end.subtract(lineupUtcOffset, 'h').toISOString(),
